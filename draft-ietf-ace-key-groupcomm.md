...
* /ace-group/GID/nodes/NODE/pub-key: one sub-resource to /ace-group/GID/nodes/NODE is implemented for each node in the group the KDC manages. These resources are identified by the node name (in this example, the node name has value "NODE"). These resources support the POST method.
...
* 'peer\_roles', may only be present if 'pub\_keys' is present. This parameter is a CBOR array, with as many elements as the public keys included in the 'pub\_keys' parameter. The i-th element of the array specifies the role (or CBOR array of roles) that the group member associated to the i-th public key in 'peer\_roles' has in the group. In particular, each array element is encoded as the second element of a scope entry, as defined in {{ssec-authorization-request}}.
...
### ace-group/GID/nodes/NODE/pub-key
...
In either case, if it wants to continue participating in the group communication, the node has to request the latest keying material from the KDC. To this end, the Client sends a CoAP GET request to the /ace-group/GID/nodes/NODE endpoint at the KDC, formatted as specified in {{node-get}}.
...
Client                                                                KDC
|                                                                       |
|-- Public Key Update Request: POST ace-group/GID/nodes/NODE/pub-key -->|
|                                                                       |
...
A node can actively request to leave the group. In this case, the Client sends a CoAP DELETE request to the endpoint /ace-group/GID/nodes/NODE at the KDC, where GID is the group identifier and NODE is the node's name, formatted as defined in {{node-delete}}
